...
export default function CustomerDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<CustomerFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-customer-field')
      if (rawPayload) {
        try {
  ...
  function findFieldAtCell(config: CustomerFormCustomizationConfig, section: string, column: number, row: number) {
  ...
  function moveFieldToCell(prev: CustomerFormCustomizationConfig, fieldId: CustomerFormFieldKey, nextSection: string, nextColumn: number, nextRow: number) {
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-customer-field', JSON.stringify(payload))
  ...
  function updateFormColumns(nextCount: number) {
    ...
    setLayout((prev) => ({
      ...
      ) as CustomerFormCustomizationConfig['fields'],
    }))
  }
  ...
  function toggleVisible(fieldId: CustomerFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: CustomerFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/customer-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { customerCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save customer form layout')
        return
      }
      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save customer form requirements')
        return
      }
      window.location.assign(detailHref)
    } catch {
      setError('Unable to save customer form customization')
    } finally {
      setSaving(false)
    }
  }
      ...
      <div className="mb-5 flex flex-wrap items-end gap-3">
        <label className="space-y-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
          ...
          <select value={layout.formColumns} onChange={(event) => updateFormColumns(Number(event.target.value))} className="rounded-md border bg-transparent px-3 py-2 text-sm text-white" style={{ borderColor: 'var(--border-muted)' }}>
            {[1, 2, 3, 4].map((count) => (
              <option key={count} value={count} style={{ backgroundColor: 'var(--card-elevated)', color: '#ffffff' }}>{count}</option>
            ))}
          </select>
        </label>
        <label className="space-y-1 text-sm" style={{ color: 'var(--text-secondary)' }}>
                  ...
                  Array.from({ length: layout.formColumns }, (_, columnIndex) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ borderColor: 'var(--border-muted)', backgroundColor: fieldConfig.visible ? 'var(--card)' : 'rgba(255,255,255,0.02)', opacity: fieldConfig.visible ? 1 : 0.72 }}
                        onDragOver={handleDragOver}
                        onDrop={(event) => handleDropToCell(event, section, column, rowIndex)}
                      >
                        <div className="mb-2 flex items-start justify-between gap-3">
                          ...
                          <div
                            draggable
                            onMouseDown={() => { draggingRef.current = dragPayload; setDragging(dragPayload) }}
