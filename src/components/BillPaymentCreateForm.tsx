...
export default function BillPaymentCreateForm({ bills, onSuccess, onCancel }: { bills: { id: string; label: string }[]; onSuccess?: () => void; onCancel?: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div><label className="mb-1 block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Method</label><select value={method} onChange={e => setMethod(e.target.value)} className={inputClass} style={inputStyle}><option>check</option><option>wire</option><option>ach</option><option>credit card</option><option>cash</option></select></div>
      <div><label className="mb-1 block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Status</label><select value={status} onChange={e => setStatus(e.target.value)} className={inputClass} style={inputStyle}><option>pending</option><option>processed</option><option>cleared</option><option>void</option></select></div>
