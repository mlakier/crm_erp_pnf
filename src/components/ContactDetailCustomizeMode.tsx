...
export default function ContactDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<ContactFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-contact-field')
      if (rawPayload) {
        try {
  ...
  function findFieldAtCell(config: ContactFormCustomizationConfig, section: string, column: number, row: number) {
  ...
  function moveFieldToCell(prev: ContactFormCustomizationConfig, fieldId: ContactFormFieldKey, nextSection: string, nextColumn: number, nextRow: number) {
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-contact-field', JSON.stringify(payload))
  ...
  function updateFormColumns(nextCount: number) {
    ...
    setLayout((prev) => ({
      ...
      ) as ContactFormCustomizationConfig['fields'],
    }))
  }
  ...
  function toggleVisible(fieldId: ContactFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: ContactFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/contact-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { contactCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save contact form layout')
        return
      }
      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save contact form requirements')
        return
      }
      window.location.assign(detailHref)
    } catch {
      setError('Unable to save contact form customization')
    } finally {
      setSaving(false)
    }
  }
