...
export default function FulfillmentCreateForm({ salesOrders, onSuccess, onCancel }: { salesOrders: { id: string; label: string }[]; onSuccess?: () => void; onCancel?: () => void }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div><label className="mb-1 block text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Status</label><select value={status} onChange={e => setStatus(e.target.value)} className={inputClass} style={inputStyle}><option>pending</option><option>shipped</option><option>delivered</option><option>cancelled</option></select></div>
