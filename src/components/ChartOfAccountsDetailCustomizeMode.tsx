...
export default function ChartOfAccountsDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<ChartOfAccountsFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-chart-of-accounts-field')
      if (rawPayload) {
        try {
  ...
  function moveFieldToCell(
    prev: ChartOfAccountsFormCustomizationConfig,
    fieldId: ChartOfAccountsFormFieldKey,
    nextSection: string,
    nextColumn: number,
    nextRow: number
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-chart-of-accounts-field', JSON.stringify(payload))
  ...
  function toggleVisible(fieldId: ChartOfAccountsFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: ChartOfAccountsFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/chart-of-accounts-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { chartOfAccountCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save chart of accounts form layout')
        return
      }

      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save chart of accounts form requirements')
        return
      }

      window.location.assign(detailHref)
    } catch {
      setError('Unable to save chart of accounts form customization')
    } finally {
      setSaving(false)
    }
  }
                  ...
                  {editingSectionName === section ? (
                    <div className="flex flex-wrap items-center gap-2">
                      <input
                        ...
                        style={{ borderColor: 'var(--border-muted)' }}
                      />
                      <button type="button" onClick={() => saveSectionName(section)} className="rounded-md px-2 py-1 text-xs font-semibold text-white" style={{ backgroundColor: 'var(--accent-primary-strong)' }}>
                        Save
                      </button>
                      <button type="button" onClick={cancelEditingSection} className="rounded-md border px-2 py-1 text-xs" style={{ borderColor: 'var(--border-muted)', color: 'var(--text-secondary)' }}>
                        Cancel
                      </button>
                    </div>
                  ) : (
