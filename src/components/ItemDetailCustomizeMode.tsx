...
export default function ItemDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<ItemFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-item-field')
      if (rawPayload) {
        try {
  ...
  function moveFieldToCell(
    prev: ItemFormCustomizationConfig,
    fieldId: ItemFormFieldKey,
    nextSection: string,
    nextColumn: number,
    nextRow: number
  ...
  function handleDragStart(
    ...
    event.dataTransfer.setData('application/x-item-field', JSON.stringify(payload))
  ...
  function toggleVisible(fieldId: ItemFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: ItemFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/item-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { itemCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save item form layout')
        return
      }

      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save item form requirements')
        return
      }

      window.location.assign(detailHref)
    } catch {
      setError('Unable to save item form customization')
    } finally {
      setSaving(false)
    }
  }
                      ...
                      >
                        ...
                        <div className="mb-3 flex items-center gap-4">
                          ...
                          <label className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
                            <input
                              type="checkbox"
                              checked={Boolean(requirements[field.id])}
