...
export default function RoleDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<RoleFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-role-field')
      if (rawPayload) {
        try {
  ...
  function findFieldAtCell(config: RoleFormCustomizationConfig, section: string, column: number, row: number) {
  ...
  function moveFieldToCell(prev: RoleFormCustomizationConfig, fieldId: RoleFormFieldKey, nextSection: string, nextColumn: number, nextRow: number) {
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-role-field', JSON.stringify(payload))
  ...
  function updateFormColumns(nextCount: number) {
    ...
    setLayout((prev) => ({
      ...
      ) as RoleFormCustomizationConfig['fields'],
    }))
  }
  ...
  function toggleVisible(fieldId: RoleFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: RoleFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/role-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { roleCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save role form layout')
        return
      }
      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save role form requirements')
        return
      }
      window.location.assign(detailHref)
    } catch {
      setError('Unable to save role form customization')
    } finally {
      setSaving(false)
    }
  }
