...
type DragState = {
  fieldId: DepartmentFormFieldKey
  section: string
  column: number
  row: number
} | null
...
export default function DepartmentDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<DepartmentFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-department-field')
      if (rawPayload) {
        try {
  ...
  function findFieldAtCell(config: DepartmentFormCustomizationConfig, section: string, column: number, row: number) {
  ...
  function moveFieldToCell(
    prev: DepartmentFormCustomizationConfig,
    fieldId: DepartmentFormFieldKey,
    nextSection: string,
    nextColumn: number,
    nextRow: number
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-department-field', JSON.stringify(payload))
  ...
  function updateFormColumns(nextCount: number) {
    ...
    setLayout((prev) => ({
      ...
      ) as DepartmentFormCustomizationConfig['fields'],
    }))
  }
  ...
  function toggleVisible(fieldId: DepartmentFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: DepartmentFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/department-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { departmentCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save department form layout')
        return
      }

      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save department form requirements')
        return
      }

      window.location.assign(detailHref)
    } catch {
      setError('Unable to save department form customization')
    } finally {
      setSaving(false)
    }
  }
