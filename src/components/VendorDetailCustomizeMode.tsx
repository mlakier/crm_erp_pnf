...
export default function VendorDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<VendorFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-vendor-field')
      if (rawPayload) {
        try {
  ...
  function findFieldAtCell(config: VendorFormCustomizationConfig, section: string, column: number, row: number) {
  ...
  function moveFieldToCell(prev: VendorFormCustomizationConfig, fieldId: VendorFormFieldKey, nextSection: string, nextColumn: number, nextRow: number) {
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-vendor-field', JSON.stringify(payload))
  ...
  function updateFormColumns(nextCount: number) {
    ...
    setLayout((prev) => ({
      ...
      ) as VendorFormCustomizationConfig['fields'],
    }))
  }
  ...
  function toggleVisible(fieldId: VendorFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: VendorFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/vendor-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { vendorCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save vendor form layout')
        return
      }
      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save vendor form requirements')
        return
      }
      window.location.assign(detailHref)
    } catch {
      setError('Unable to save vendor form customization')
    } finally {
      setSaving(false)
    }
  }
