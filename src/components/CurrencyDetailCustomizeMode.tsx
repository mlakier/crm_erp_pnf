...
export default function CurrencyDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<CurrencyFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-currency-field')
      if (rawPayload) {
        try {
  ...
  function moveFieldToCell(
    prev: CurrencyFormCustomizationConfig,
    fieldId: CurrencyFormFieldKey,
    nextSection: string,
    nextColumn: number,
    nextRow: number
  ...
  function handleDragStart(
    ...
    event.dataTransfer.setData('application/x-currency-field', JSON.stringify(payload))
  ...
  function toggleVisible(fieldId: CurrencyFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: CurrencyFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/currency-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { currencyCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save currency form layout')
        return
      }

      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save currency form requirements')
        return
      }

      window.location.assign(detailHref)
    } catch {
      setError('Unable to save currency form customization')
    } finally {
      setSaving(false)
    }
  }
