...
type DragState = {
  fieldId: SubsidiaryFormFieldKey
  section: string
  column: number
  row: number
} | null
...
export default function SubsidiaryDetailCustomizeMode({
  ...
  const [layout, setLayout] = useState<SubsidiaryFormCustomizationConfig>(initialLayout)
  ...
  function resolveDragPayload(event?: React.DragEvent<HTMLElement>) {
    if (event) {
      const rawPayload = event.dataTransfer.getData('application/x-subsidiary-field')
      if (rawPayload) {
        try {
  ...
  function findFieldAtCell(config: SubsidiaryFormCustomizationConfig, section: string, column: number, row: number) {
  ...
  function moveFieldToCell(prev: SubsidiaryFormCustomizationConfig, fieldId: SubsidiaryFormFieldKey, nextSection: string, nextColumn: number, nextRow: number) {
  ...
  function handleDragStart(event: React.DragEvent<HTMLElement>, payload: DragState) {
    ...
    event.dataTransfer.setData('application/x-subsidiary-field', JSON.stringify(payload))
  ...
  function toggleVisible(fieldId: SubsidiaryFormFieldKey) {
    setLayout((prev) => ({
      ...prev,
      fields: {
        ...prev.fields,
  ...
  function toggleRequired(fieldId: SubsidiaryFormFieldKey) {
  ...
  async function saveCustomization() {
    ...
    try {
      const [layoutResponse, requirementsResponse] = await Promise.all([
        fetch('/api/config/subsidiary-form-customization', {
          method: 'POST',
        ...
        fetch('/api/config/form-requirements', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ config: { subsidiaryCreate: requirements } }),
        }),
      ])
      ...
      if (!layoutResponse.ok) {
        setError(layoutBody?.error ?? 'Unable to save subsidiary form layout')
        return
      }
      if (!requirementsResponse.ok) {
        setError(requirementsBody?.error ?? 'Unable to save subsidiary form requirements')
        return
      }
      window.location.assign(detailHref)
    } catch {
      setError('Unable to save subsidiary form customization')
    } finally {
      setSaving(false)
    }
  }
