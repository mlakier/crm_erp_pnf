...
export type CompanyPreferencesSection =
  | 'money-display'
  | 'transaction-status-colors'
  | 'master-data-id-settings'
  | 'crm-id-settings'
  | 'transaction-id-settings'

type SectionMeta = {
  id: CompanyPreferencesSection
  href: string
  label: string
  description: string
}
...
const SECTION_META: SectionMeta[] = [
  {
    ...
    label: 'Money Display',
    description: 'Control locale, currency, date, zero, and negative number display rules.',
  },
  {
    id: 'transaction-status-colors',
    href: '/company-preferences/transaction-status-colors',
    label: 'Transaction Status Colors',
    description: 'Manage company-level status color mapping for transactions.',
  },
  {
    id: 'master-data-id-settings',
    href: '/company-preferences/master-data-id-settings',
    label: 'Master Data Id Settings',
    description: 'Set prefixes and numbering rules for users, items, customers, vendors, and other master records.',
  },
  {
    id: 'crm-id-settings',
    href: '/company-preferences/crm-id-settings',
    label: 'CRM Id Settings',
    description: 'Set prefixes and numbering rules for leads and opportunities.',
  },
  {
    id: 'transaction-id-settings',
    href: '/company-preferences/transaction-id-settings',
    label: 'Transaction Id Settings',
    description: 'Set numbering rules for quotes, sales orders, invoices, purchasing, and journals.',
  },
]
...
export default function CompanyPreferencesSectionPage({
  section,
}: {
  section: CompanyPreferencesSection
}) {
      ...
      try {
        ...
        const nextSettings = {
          ...
          transactionStatusColors: {
            salesOrder: {
              ...DEFAULT_TRANSACTION_STATUS_COLOR_SETTINGS.salesOrder,
              ...(body.transactionStatusColors?.salesOrder ?? {}),
            },
          },
        }
  ...
  const activeSection = SECTION_META.find((entry) => entry.id === section) ?? SECTION_META[0]
  ...
  function renderMoneyDisplaySection() {
    return (
      <section
        ...
        >
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-medium" style={{ color: getNegativeAmountColor(draftSettings.moneySettings) ?? '#ffffff' }}>
  ...
  function renderTransactionStatusColorSection() {
    return (
      <section
        className="rounded-xl border p-5"
        style={{ borderColor: 'var(--border-muted)', backgroundColor: 'var(--card-elevated)' }}
      >
        ...
        <p className="mt-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
          Transaction status and stage colors now live in Manage Lists. Update the relevant managed list to control badge and stat-card color coding.
        </p>
        <div className="mt-4 rounded-lg border p-4" style={{ borderColor: 'var(--border-muted)', backgroundColor: 'var(--card)' }}>
          <p className="text-sm text-white">Use managed lists like `OPP-STAGE`, `LEAD-STATUS`, `QUOTE-STATUS`, `SO-STATUS`, and `INV-STATUS` for transaction color settings.</p>
          <p className="mt-2 text-sm" style={{ color: 'var(--text-muted)' }}>
            These colors now drive the detail-page badges and status stat cards directly from the list values.
          </p>
        </div>
      </section>
    )
  ...
  function renderIdSettingsSection(group: 'Master Data' | 'CRM' | 'Transactions') {
  ...
  function renderSection() {
    if (section === 'money-display') return renderMoneyDisplaySection()
    if (section === 'transaction-status-colors') return renderTransactionStatusColorSection()
    if (section === 'master-data-id-settings') return renderIdSettingsSection('Master Data')
    if (section === 'crm-id-settings') return renderIdSettingsSection('CRM')
    return renderIdSettingsSection('Transactions')
  }
  ...
  return (
    <div className="min-h-full px-8 py-8">
      ...
      >
        <div>
          <Link href="/company-preferences" className="text-sm hover:underline" style={{ color: 'var(--accent-primary-strong)' }}>
            {'<- Back to Company Prefs'}
          </Link>
          <h1 className="mt-4 text-xl font-semibold text-white">{activeSection.label}</h1>
          <p className="mt-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
          ...
          {SECTION_META.map((entry) => {
            ...
            return (
              <Link
                key={entry.id}
                href={entry.href}
                className="rounded-xl border p-4 transition hover:border-white/30"
                style={{
                  borderColor: isActive ? 'var(--accent-primary-strong)' : 'var(--border-muted)',
                  backgroundColor: isActive ? 'rgba(59,130,246,0.10)' : 'var(--card-elevated)',
                }}
              >
                <p className="text-sm font-semibold" style={{ color: isActive ? 'var(--accent-primary-strong)' : '#ffffff' }}>
                  {entry.label}
                </p>
