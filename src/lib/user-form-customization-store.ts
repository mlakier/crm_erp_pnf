...
const STORE_PATH = path.join(process.cwd(), 'config', 'user-form-customization.json')

function cloneDefaults(): UserFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultUserFormCustomization())) as UserFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: UserFormCustomizationConfig): UserFormCustomizationConfig {
  const nextConfig: UserFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      USER_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }])
    ) as UserFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = USER_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<UserFormCustomizationConfig>): UserFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<UserFormFieldKey, Partial<UserFormCustomizationConfig['fields'][UserFormFieldKey]>>>
    : {}
...
export async function loadUserFormCustomization(): Promise<UserFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<UserFormCustomizationConfig>
...
export async function saveUserFormCustomization(nextConfig: UserFormCustomizationConfig): Promise<UserFormCustomizationConfig> {
