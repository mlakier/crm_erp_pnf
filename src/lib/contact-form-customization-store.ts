...
const STORE_PATH = path.join(process.cwd(), 'config', 'contact-form-customization.json')

function cloneDefaults(): ContactFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultContactFormCustomization())) as ContactFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: ContactFormCustomizationConfig): ContactFormCustomizationConfig {
  const nextConfig: ContactFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      CONTACT_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }])
    ) as ContactFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = CONTACT_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<ContactFormCustomizationConfig>): ContactFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<ContactFormFieldKey, Partial<ContactFormCustomizationConfig['fields'][ContactFormFieldKey]>>>
    : {}
  ...
  const normalizedStatCards = statCardOverrides
    .map((entry, index) => {
      ...
      if (!root) return null
      const metric = String(root.metric ?? '').trim() as ContactStatCardMetric
      if (!allowedMetrics.has(metric)) return null
      const size: 'sm' | 'md' | 'lg' = root.size === 'sm' || root.size === 'lg' ? root.size : 'md'
      return {
        id: String(root.id ?? `contact-stat-${metric}`),
        metric,
...
export async function loadContactFormCustomization(): Promise<ContactFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<ContactFormCustomizationConfig>
...
export async function saveContactFormCustomization(nextConfig: ContactFormCustomizationConfig): Promise<ContactFormCustomizationConfig> {
