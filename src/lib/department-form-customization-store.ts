...
import {
  ...
  type DepartmentStatCardMetric,
} from '@/lib/department-form-customization'

const STORE_PATH = path.join(process.cwd(), 'config', 'department-form-customization.json')

function cloneDefaults(): DepartmentFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultDepartmentFormCustomization())) as DepartmentFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: DepartmentFormCustomizationConfig): DepartmentFormCustomizationConfig {
  const nextConfig: DepartmentFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      DEPARTMENT_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }])
    ) as DepartmentFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = DEPARTMENT_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<DepartmentFormCustomizationConfig>): DepartmentFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<DepartmentFormFieldKey, Partial<DepartmentFormCustomizationConfig['fields'][DepartmentFormFieldKey]>>>
    : {}
  const statCardOverrides = Array.isArray(overrides.statCards) ? overrides.statCards : []

  for (const field of DEPARTMENT_FORM_FIELDS) {
...
export async function loadDepartmentFormCustomization(): Promise<DepartmentFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<DepartmentFormCustomizationConfig>
...
export async function saveDepartmentFormCustomization(nextConfig: DepartmentFormCustomizationConfig): Promise<DepartmentFormCustomizationConfig> {
