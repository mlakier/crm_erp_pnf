...
import {
  ...
  type VendorStatCardMetric,
} from '@/lib/vendor-form-customization'

const STORE_PATH = path.join(process.cwd(), 'config', 'vendor-form-customization.json')

function cloneDefaults(): VendorFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultVendorFormCustomization())) as VendorFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: VendorFormCustomizationConfig): VendorFormCustomizationConfig {
  const nextConfig: VendorFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      VENDOR_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }])
    ) as VendorFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = VENDOR_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<VendorFormCustomizationConfig>): VendorFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<VendorFormFieldKey, Partial<VendorFormCustomizationConfig['fields'][VendorFormFieldKey]>>>
    : {}
  const statCardOverrides = Array.isArray(overrides.statCards) ? overrides.statCards : []

  for (const field of VENDOR_FORM_FIELDS) {
...
export async function loadVendorFormCustomization(): Promise<VendorFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<VendorFormCustomizationConfig>
...
export async function saveVendorFormCustomization(nextConfig: VendorFormCustomizationConfig): Promise<VendorFormCustomizationConfig> {
