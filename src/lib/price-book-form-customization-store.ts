...
const STORE_PATH = path.join(process.cwd(), 'config', 'price-book-form-customization.json')

function cloneDefaults(): PriceBookFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultPriceBookFormCustomization())) as PriceBookFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: PriceBookFormCustomizationConfig): PriceBookFormCustomizationConfig {
  const nextConfig: PriceBookFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      PRICE_BOOK_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }]),
    ) as PriceBookFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = PRICE_BOOK_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<PriceBookFormCustomizationConfig>): PriceBookFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<PriceBookFormFieldKey, Partial<PriceBookFormCustomizationConfig['fields'][PriceBookFormFieldKey]>>>
    : {}
  const statCardOverrides = Array.isArray(overrides.statCards) ? overrides.statCards : []

  for (const field of PRICE_BOOK_FORM_FIELDS) {
  ...
  const normalizedStatCards = statCardOverrides
    .map((entry, index) => {
      ...
      const metric = String(root.metric ?? '').trim() as PriceBookStatCardMetric
      if (!allowedMetrics.has(metric)) return null

      const size: 'sm' | 'md' | 'lg' = root.size === 'sm' || root.size === 'lg' ? root.size : 'md'
      return {
        id: String(root.id ?? `price-book-stat-${metric}`),
        metric,
...
export async function loadPriceBookFormCustomization(): Promise<PriceBookFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<PriceBookFormCustomizationConfig>
...
export async function savePriceBookFormCustomization(nextConfig: PriceBookFormCustomizationConfig): Promise<PriceBookFormCustomizationConfig> {
