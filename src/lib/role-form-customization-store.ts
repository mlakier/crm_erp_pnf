...
function cloneDefaults(): RoleFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultRoleFormCustomization())) as RoleFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: RoleFormCustomizationConfig): RoleFormCustomizationConfig {
  const nextConfig: RoleFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      ROLE_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }])
    ) as RoleFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = ROLE_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<RoleFormCustomizationConfig>): RoleFormCustomizationConfig {
...
export async function loadRoleFormCustomization(): Promise<RoleFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<RoleFormCustomizationConfig>
...
export async function saveRoleFormCustomization(nextConfig: RoleFormCustomizationConfig): Promise<RoleFormCustomizationConfig> {
