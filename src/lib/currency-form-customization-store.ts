...
const STORE_PATH = path.join(process.cwd(), 'config', 'currency-form-customization.json')

function cloneDefaults(): CurrencyFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultCurrencyFormCustomization())) as CurrencyFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: CurrencyFormCustomizationConfig): CurrencyFormCustomizationConfig {
  const nextConfig: CurrencyFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      CURRENCY_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }])
    ) as CurrencyFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = CURRENCY_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<CurrencyFormCustomizationConfig>): CurrencyFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<CurrencyFormFieldKey, Partial<CurrencyFormCustomizationConfig['fields'][CurrencyFormFieldKey]>>>
    : {}
  const statCardOverrides = Array.isArray(overrides.statCards) ? overrides.statCards : []

  for (const field of CURRENCY_FORM_FIELDS) {
  ...
  const normalizedStatCards = statCardOverrides
    .map((entry, index) => {
      ...
      if (!root) return null
      const metric = String(root.metric ?? '').trim() as CurrencyStatCardMetric
      if (!allowedMetrics.has(metric)) return null
      return {
        id: String(root.id ?? `currency-stat-${metric}`),
        metric,
...
export async function loadCurrencyFormCustomization(): Promise<CurrencyFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<CurrencyFormCustomizationConfig>
...
export async function saveCurrencyFormCustomization(nextConfig: CurrencyFormCustomizationConfig): Promise<CurrencyFormCustomizationConfig> {
