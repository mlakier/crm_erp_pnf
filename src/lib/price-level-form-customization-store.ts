...
import {
  PRICE_LEVEL_FORM_FIELDS,
  PRICE_LEVEL_STAT_CARDS,
  defaultPriceLevelFormCustomization,
  type PriceLevelFormCustomizationConfig,
  type PriceLevelFormFieldKey,
  type PriceLevelStatCardMetric,
} from '@/lib/price-level-form-customization'

const STORE_PATH = path.join(process.cwd(), 'config', 'price-level-form-customization.json')

function cloneDefaults(): PriceLevelFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultPriceLevelFormCustomization())) as PriceLevelFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: PriceLevelFormCustomizationConfig): PriceLevelFormCustomizationConfig {
  const nextConfig: PriceLevelFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      PRICE_LEVEL_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }]),
    ) as PriceLevelFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = PRICE_LEVEL_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<PriceLevelFormCustomizationConfig>): PriceLevelFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<PriceLevelFormFieldKey, Partial<PriceLevelFormCustomizationConfig['fields'][PriceLevelFormFieldKey]>>>
    : {}
  const statCardOverrides = Array.isArray(overrides.statCards) ? overrides.statCards : []

  for (const field of PRICE_LEVEL_FORM_FIELDS) {
...
export async function loadPriceLevelFormCustomization(): Promise<PriceLevelFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<PriceLevelFormCustomizationConfig>
...
export async function savePriceLevelFormCustomization(nextConfig: PriceLevelFormCustomizationConfig): Promise<PriceLevelFormCustomizationConfig> {
