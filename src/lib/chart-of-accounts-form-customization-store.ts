...
const STORE_PATH = path.join(process.cwd(), 'config', 'chart-of-accounts-form-customization.json')

function cloneDefaults(): ChartOfAccountsFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultChartOfAccountsFormCustomization())) as ChartOfAccountsFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: ChartOfAccountsFormCustomizationConfig): ChartOfAccountsFormCustomizationConfig {
  const nextConfig: ChartOfAccountsFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      CHART_OF_ACCOUNTS_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }])
    ) as ChartOfAccountsFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = CHART_OF_ACCOUNTS_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<ChartOfAccountsFormCustomizationConfig>): ChartOfAccountsFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<ChartOfAccountsFormFieldKey, Partial<ChartOfAccountsFormCustomizationConfig['fields'][ChartOfAccountsFormFieldKey]>>>
    : {}
  const statCardOverrides = Array.isArray(overrides.statCards) ? overrides.statCards : []

  for (const field of CHART_OF_ACCOUNTS_FORM_FIELDS) {
...
export async function loadChartOfAccountsFormCustomization(): Promise<ChartOfAccountsFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<ChartOfAccountsFormCustomizationConfig>
...
export async function saveChartOfAccountsFormCustomization(nextConfig: ChartOfAccountsFormCustomizationConfig): Promise<ChartOfAccountsFormCustomizationConfig> {
