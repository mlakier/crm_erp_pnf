...
import {
  ...
  type CustomerStatCardMetric,
} from '@/lib/customer-form-customization'

const STORE_PATH = path.join(process.cwd(), 'config', 'customer-form-customization.json')

function cloneDefaults(): CustomerFormCustomizationConfig {
  return JSON.parse(JSON.stringify(defaultCustomerFormCustomization())) as CustomerFormCustomizationConfig
}
...
function normalizeFieldPlacements(config: CustomerFormCustomizationConfig): CustomerFormCustomizationConfig {
  const nextConfig: CustomerFormCustomizationConfig = {
    ...config,
    sectionRows: { ...config.sectionRows },
    fields: Object.fromEntries(
      CUSTOMER_FORM_FIELDS.map((field) => [field.id, { ...config.fields[field.id] }])
    ) as CustomerFormCustomizationConfig['fields'],
  }

  for (const section of nextConfig.sections) {
    const sectionFields = CUSTOMER_FORM_FIELDS.filter((field) => nextConfig.fields[field.id].section === section)
...
function mergeWithDefaults(overrides: Partial<CustomerFormCustomizationConfig>): CustomerFormCustomizationConfig {
  ...
  const fieldOverrides = overrides.fields && typeof overrides.fields === 'object'
    ? overrides.fields as Partial<Record<CustomerFormFieldKey, Partial<CustomerFormCustomizationConfig['fields'][CustomerFormFieldKey]>>>
    : {}
  ...
  const allowedMetrics = new Set<CustomerStatCardMetric>(CUSTOMER_STAT_CARDS.map((card) => card.id))
  const normalizedStatCards = statCardOverrides
    .map((entry, index) => {
      const root = entry && typeof entry === 'object' ? entry as Record<string, unknown> : null
      if (!root) return null

      const metric = String(root.metric ?? '').trim() as CustomerStatCardMetric
      if (!allowedMetrics.has(metric)) return null

      return {
        id: String(root.id ?? `customer-stat-${metric}`),
        metric,
...
export async function loadCustomerFormCustomization(): Promise<CustomerFormCustomizationConfig> {
  try {
    const raw = await fs.readFile(STORE_PATH, 'utf8')
    const parsed = JSON.parse(raw) as Partial<CustomerFormCustomizationConfig>
...
export async function saveCustomerFormCustomization(nextConfig: CustomerFormCustomizationConfig): Promise<CustomerFormCustomizationConfig> {
