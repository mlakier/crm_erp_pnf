...
export default async function FulfillmentsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; sort?: string; page?: string }> }) {
  ...
  const [totalRows, salesOrders, companySettings, cabinetFiles] = await Promise.all([
  ...
  const rows = await db.fulfillment.findMany({ where, include: { salesOrder: { include: { customer: true } }, entity: true, currency: true }, orderBy, skip: pagination.skip, take: pagination.pageSize })
    ...
    <div className="min-h-full px-8 py-8">
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <CreateModalButton buttonLabel="New Fulfillment" title="New Fulfillment">
          <FulfillmentCreateForm salesOrders={salesOrders.map((so: any) => ({ id: so.id, label: so.number + ' - ' + so.customer.name }))} />
        </CreateModalButton>
      </div>
      ...
      <section className="overflow-hidden rounded-2xl border" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border-muted)' }}>
        <form className="border-b px-6 py-4" method="get" style={{ borderColor: 'var(--border-muted)' }}>
          ...
          <div className="flex gap-3 items-center flex-nowrap">
            <input type="text" name="q" defaultValue={params.q ?? ''} placeholder="Search fulfillment #, status" className="flex-1 min-w-0 rounded-md border bg-transparent px-3 py-2 text-sm text-white" style={{ borderColor: 'var(--border-muted)' }} />
        ...
        <div className="overflow-x-auto" data-column-selector-table="fulfillments-list">
          <table className="min-w-full" id="fulfillments-list">
            <thead><tr style={{ borderBottom: '1px solid var(--border-muted)' }}>
              {FULFILLMENT_COLUMNS.map(c => <th key={c.id} data-column={c.id} className="sticky top-0 z-10 px-4 py-2 text-left text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--text-muted)', backgroundColor: 'var(--card)' }}>{c.label}</th>)}
            </tr></thead>
            <tbody>
              {rows.length === 0 ? (
                ...
                <tr key={row.id} style={i < rows.length - 1 ? { borderBottom: '1px solid var(--border-muted)' } : {}}>
                  ...
                  <td data-column="actions" className="px-4 py-2 text-sm"><span className="flex items-center gap-2">
                    <EditButton id={row.id} endpoint="/api/fulfillments" fields={[{ name: 'status', label: 'Status', type: 'select', value: row.status, options: ['pending','shipped','delivered','cancelled'] }, { name: 'notes', label: 'Notes', type: 'text', value: row.notes ?? '' }]} />
