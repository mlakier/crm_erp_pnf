...
export default async function ReceiptsPage({
  ...
  // Fetch purchase orders for the create form
  const purchaseOrders = await prisma.purchaseOrder.findMany({ select: { id: true, number: true }, orderBy: { createdAt: 'desc' }, take: 200 })
  ...
  const companyLogoPages = logoFileId ? cabinetFiles.find((f: any) => f.id === logoFileId) : null
  ...
  return (
    <div className="min-h-full px-8 py-8">
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>{totalReceipts} total</p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        {STATUS_OPTIONS.map((s) => {
          ...
          const href = `/receipts?${new URLSearchParams({ ...(params.q ? { q: params.q } : {}), status: s, page: '1' }).toString()}`
          return (
            <Link
              key={s}
              href={href}
        ...
        <form className="border-b px-6 py-4" method="get" style={{ borderColor: 'var(--border-muted)' }}>
          ...
          <div className="flex gap-3 items-center flex-nowrap">
            <input
              ...
              placeholder="Search PO #, status, notes"
          ...
          <table className="min-w-full" id="receipts-list">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-muted)' }}>
                <th data-column="purchase-order" className="sticky top-0 z-10 px-4 py-2 text-left text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--text-muted)', backgroundColor: 'var(--card)' }}>Purchase Order</th>
                <th data-column="quantity" className="sticky top-0 z-10 px-4 py-2 text-left text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--text-muted)', backgroundColor: 'var(--card)' }}>Quantity</th>
                <th data-column="date" className="sticky top-0 z-10 px-4 py-2 text-left text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--text-muted)', backgroundColor: 'var(--card)' }}>Date</th>
            ...
            <tbody>
              {receipts.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-4 py-8 text-center text-sm" style={{ color: 'var(--text-muted)' }}>No receipts found.</td>
                </tr>
              ) : (
                receipts.map((receipt, index) => (
                  <tr key={receipt.id} style={index < receipts.length - 1 ? { borderBottom: '1px solid var(--border-muted)' } : {}}>
                    <td data-column="purchase-order" className="px-4 py-2 text-sm" style={{ color: 'var(--text-secondary)' }}>
                      <Link href={`/purchase-orders/${receipt.purchaseOrderId}`} className="font-medium hover:underline" style={{ color: 'var(--accent-primary-strong)' }}>
                        {receipt.purchaseOrder.number}
                      </Link>
                    </td>
                    <td data-column="quantity" className="px-4 py-2 text-sm" style={{ color: 'var(--text-secondary)' }}>{(receipt as any).quantity}</td>
                    <td data-column="date" className="px-4 py-2 text-sm" style={{ color: 'var(--text-secondary)' }}>{new Date((receipt as any).date).toLocaleDateString()}</td>
                    <td data-column="status" className="px-4 py-2 text-sm" style={{ color: 'var(--text-secondary)' }}>{(receipt as any).status}</td>
                    <td data-column="notes" className="px-4 py-2 text-sm truncate max-w-[200px]" style={{ color: 'var(--text-secondary)' }}>{(receipt as any).notes ?? '—'}</td>
                    <td data-column="created" className="px-4 py-2 text-sm" style={{ color: 'var(--text-secondary)' }}>{new Date(receipt.createdAt).toLocaleDateString()}</td>
                    <td data-column="last-modified" className="px-4 py-2 text-sm" style={{ color: 'var(--text-secondary)' }}>{new Date(receipt.updatedAt).toLocaleDateString()}</td>
                  </tr>
                ))
              )}
            </tbody>
