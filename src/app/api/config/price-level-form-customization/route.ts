...
import { loadPriceLevelFormCustomization, savePriceLevelFormCustomization } from '@/lib/price-level-form-customization-store'
import type { TransactionStatCardSize, TransactionStatCardSlot } from '@/lib/transaction-page-config'

function sanitizeInput(input: unknown): PriceLevelFormCustomizationConfig {
  const defaults = defaultPriceLevelFormCustomization()
  ...
  ) as Record<PriceLevelFormFieldKey, PriceLevelFormCustomizationConfig['fields'][PriceLevelFormFieldKey]>
  ...
  return {
    ...
    statCards: (() : Array<TransactionStatCardSlot<PriceLevelStatCardMetric>> => {
      const normalized: Array<TransactionStatCardSlot<PriceLevelStatCardMetric>> = statCardsInput
        .map((entry, index) => {
          const statRoot = entry && typeof entry === 'object' ? entry as Record<string, unknown> : null
          if (!statRoot) return null

          const metric = String(statRoot.metric ?? '').trim() as PriceLevelStatCardMetric
          ...
          return {
            id: String(statRoot.id ?? `price-level-stat-${metric}`),
            metric,
...
export async function GET() {
  try {
    const config = await loadPriceLevelFormCustomization()
    return NextResponse.json({ config, fields: PRICE_LEVEL_FORM_FIELDS })
  } catch {
    return NextResponse.json({ error: 'Failed to load price level form customization' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const saved = await savePriceLevelFormCustomization(sanitized)
    return NextResponse.json({ config: saved, fields: PRICE_LEVEL_FORM_FIELDS })
  } catch {
    return NextResponse.json({ error: 'Failed to save price level form customization' }, { status: 500 })
  }
}
