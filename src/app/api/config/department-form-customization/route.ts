...
function sanitizeInput(input: unknown): DepartmentFormCustomizationConfig {
  ...
  ) as Record<DepartmentFormFieldKey, DepartmentFormCustomizationConfig['fields'][DepartmentFormFieldKey]>
...
export async function GET() {
  try {
    const config = await loadDepartmentFormCustomization()
    return NextResponse.json({ config, fields: DEPARTMENT_FORM_FIELDS })
  } catch {
    return NextResponse.json({ error: 'Failed to load department form customization' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const saved = await saveDepartmentFormCustomization(sanitized)
    return NextResponse.json({ config: saved, fields: DEPARTMENT_FORM_FIELDS })
  } catch {
    return NextResponse.json({ error: 'Failed to save department form customization' }, { status: 500 })
  }
}
