...
import {
  ...
  type AccountingPeriodStatCardMetric,
...
function sanitizeInput(input: unknown): AccountingPeriodFormCustomizationConfig {
  ...
  const allowedMetrics = new Set<AccountingPeriodStatCardMetric>(ACCOUNTING_PERIOD_STAT_CARDS.map((card) => card.id))
  ...
  return {
    ...
    statCards: (() : Array<TransactionStatCardSlot<AccountingPeriodStatCardMetric>> => {
      const normalized: Array<TransactionStatCardSlot<AccountingPeriodStatCardMetric>> = statCardsInput
        .map((entry, index) => {
          const statRoot = entry && typeof entry === 'object' ? entry as Record<string, unknown> : null
          if (!statRoot) return null

          const metric = String(statRoot.metric ?? '').trim() as AccountingPeriodStatCardMetric
          ...
          return {
            id: String(statRoot.id ?? `accounting-period-stat-${metric}`),
            metric,
