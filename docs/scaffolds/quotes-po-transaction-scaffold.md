...
8. src/components/QuoteCreatePageClient.tsx: PO-style full-page create shell using RecordDetailPageShell + TransactionHeaderSections equivalent.
...
10. src/components/QuoteHeaderSections.tsx or reuse TransactionHeaderSections: sectioned/grid header rendering for view/edit/new.
...
21. Verification: lint changed files, tsc, route smoke test, quotes/new load, quotes/[id]?edit=1 load, quotes/[id]?customize=1 load.
   Verification should also confirm saved line-item and GL impact customization is reflected after reload in both detail view and `quotes/[id]?edit=1`, with truncation and hover fallback still usable where values are shortened.
...
- Detail edit mode reached through `quotes/[id]?edit=1` keeps the intended utility actions for existing records.
